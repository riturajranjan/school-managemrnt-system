...
export default function StudentAccountFoundationPage() {
  const { data, loading, error } = useApiResource<MyStudentProfile>("/api/me/student-profile");
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-md px-md py-3xl text-center">
      ...
      {loading ? (
        ...
        <>
          <h1 className="text-lg font-semibold text-foreground">Welcome, {data?.name}</h1>
          <p className="text-sm text-muted-foreground">
            Admission No. {data?.admissionNumber}
            {data?.classLabel ? ` · ${data.classLabel}${data.sectionLabel ? ` ${data.sectionLabel}` : ""}` : ""}
          </p>
          <div className="mt-md rounded-lg border border-dashed border-border bg-surface px-md py-md text-sm text-muted-foreground">
            Your account is set up and this is a real login — but the student portal
            (grades, homework, attendance, fees) is not built yet. It is a planned
            future phase, not something this page fakes.
          </div>
        </>
      )}
    </div>
