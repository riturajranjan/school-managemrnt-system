    ...
    <div className="flex flex-col gap-md pb-20 sm:pb-0">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Track every exam through the result pipeline — marks, verification,
          calculation, review, report cards and publication
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-sm sm:grid-cols-3">
        ...
        <Link
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Pass rates, distribution, trends
            </p>
          </div>
        </Link>
      </div>
            ...
            <div
              ...
              <div className="flex flex-wrap items-start justify-between gap-sm">
                <div className="min-w-0">
                  <Link
                    href={`/exams/${row.exam.id}`}
                    className="truncate text-sm font-semibold text-foreground outline-none hover:underline focus-visible:ring-2 focus-visible:ring-ring">
                    {row.exam.name}
                  </Link>
                  <p className="mt-0.5 flex flex-wrap items-center gap-x-sm gap-y-0.5 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <Calendar className="size-3" />{" "}
                      {formatDate(row.exam.startDate)}
                    </span>
                    <span className="flex items-center gap-1">
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-end gap-xs">
                  {row.inconsistencies.length > 0 && (
                    <Badge tone="warning">
                      ...
                      {row.inconsistencies.length} inconsistenc
                      {row.inconsistencies.length === 1 ? "y" : "ies"}
                    </Badge>
                  )}
                  {pubBadge.tone === "success" && (
                    <UploadCloud className="size-3.5 text-success" />
                  )}
        ...
        {relevant.length === 0 && (
          <div className="flex flex-col items-center gap-sm rounded-lg border border-dashed border-border bg-surface px-md py-2xl text-center">
            ...
            <div className="mx-auto flex  flex-col gap-1">
              <p className="text-sm font-semibold text-foreground">
                No exams are at the results stage yet
              </p>
              <p className="text-sm text-muted-foreground">
                Once an exam reaches marks entry, verification or beyond, it
                appears here with its full result pipeline.
              </p>
            </div>
          </div>
        )}
