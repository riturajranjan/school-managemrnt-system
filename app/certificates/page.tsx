...
export default function CertificatesHubPage() {
  ...
  const counts = useMemo(() => ({
    student: db.generatedDocuments.filter((d) => d.kind === "student-certificate").length,
    staff: db.generatedDocuments.filter((d) => d.kind === "staff-certificate").length,
    activity: db.generatedDocuments.filter((d) => d.kind === "activity-certificate").length,
  }), [db.generatedDocuments]);

  if (!can("documents.view")) return <PermissionDenied action="view certificates" role={roleLabels[role]} backHref="/documents" />;
  ...
  return (
    <div className="flex flex-col gap-md pb-20 sm:pb-0">
      ...
      <div className="grid grid-cols-1 gap-sm sm:grid-cols-3">
        <Link href="/certificates/student"><StatTile label="Student certificates" value={String(counts.student)} icon={GraduationCap} tone="info" /></Link>
        <Link href="/certificates/staff"><StatTile label="Staff certificates" value={String(counts.staff)} icon={UserCog} tone="neutral" /></Link>
        <Link href="/certificates/activities"><StatTile label="Activity certificates" value={String(counts.activity)} icon={Award} tone="success" /></Link>
      </div>
      ...
      <p className="rounded-md border border-primary/25 bg-primary/5 p-sm text-xs text-primary">Character &amp; conduct certificates use only preset, school-approved language — no automatic personal judgments are generated.</p>
    </div>
  );
}
