...
export default function StaffCertificatesPage() {
  ...
  if (!can("documents.view")) return <PermissionDenied action="view Staff certificates" role={roleLabels[role]} backHref="/certificates" />;
  return (
    <div className="flex flex-col gap-md pb-20 sm:pb-0">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2"><Button asChild size="sm" variant="ghost"><Link href="/certificates"><ArrowLeft className="size-4" /></Link></Button><div><h1 className="text-lg font-semibold text-foreground">Staff certificates</h1><p className="text-xs text-muted-foreground">Preview and reprint issued certificates</p></div></div>
      ...
      <GeneratedDocList kinds={["staff-certificate"]} emptyLabel="No staff certificates yet." />
    </div>
  );
}
