...
export default function CertificateTemplatesPage() {
  ...
  if (!can("documents.view")) return <PermissionDenied action="view certificate templates" role={roleLabels[role]} backHref="/certificates" />;
  const templates = db.documentTemplates.filter((t) => t.kind === "student-certificate" || t.kind === "staff-certificate" || t.kind === "activity-certificate");
  return (
      ...
      <div className="grid grid-cols-1 gap-sm sm:grid-cols-2 lg:grid-cols-3">
        {templates.map((t) => (
          <Link key={t.id} href={`/documents/templates/${t.id}`} className="surface-3d flex flex-col gap-sm rounded-lg border border-border bg-surface p-md transition hover:border-primary/40">
            <div className="flex h-20 items-center justify-center rounded-md border" style={{ borderColor: t.accent, background: `${t.accent}10` }}><span className="text-sm font-bold" style={{ color: t.accent }}>{documentTypeLabels[t.docType]}</span></div>
            <div className="flex items-start justify-between gap-sm"><div className="min-w-0"><p className="truncate text-sm font-semibold text-foreground">{t.name}</p><p className="truncate text-xs text-muted-foreground">{paperSizeLabels[t.paperSize]} · {t.orientation}</p></div><Badge tone={t.status === "active" ? "success" : "neutral"}>{t.status}</Badge></div>
