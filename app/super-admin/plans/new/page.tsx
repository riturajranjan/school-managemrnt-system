  ...
  return (
    <div className="mx-auto flex  flex-col gap-md pb-20 sm:pb-0">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-lg font-semibold text-foreground">New plan</h1>
          <p className="text-xs text-muted-foreground">
            Draft a subscription plan (frontend only)
          </p>
        </div>
      </div>
      <div className="surface-3d flex flex-col gap-sm rounded-lg border border-border bg-surface p-md">
        ...
        <div className="grid grid-cols-2 gap-sm">
          ...
          <div>
            <Label htmlFor="p-students">Student limit</Label>
            <Input
              id="p-students"
              type="number"
              value={students}
        ...
        <div>
          ...
          <Textarea id="p-desc" rows={2} placeholder="Short description" />
        </div>
        {saved && (
          <p className="rounded-md border border-success/30 bg-success/8 p-sm text-xs text-success">
            Plan drafted (simulation — not persisted).
          </p>
        )}
