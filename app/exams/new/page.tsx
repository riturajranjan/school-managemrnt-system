...
const examTypeOptions: { value: ExamType; label: string }[] = [
  { value: "unit-test", label: "Unit test" }, { value: "weekly-test", label: "Weekly test" }, { value: "monthly-test", label: "Monthly test" },
  { value: "midterm", label: "Midterm" }, { value: "half-yearly", label: "Half-yearly" }, { value: "annual", label: "Annual" },
  { value: "pre-board", label: "Pre-board" }, { value: "board", label: "Board examination" }, { value: "practical", label: "Practical" },
  { value: "oral", label: "Oral" }, { value: "assignment", label: "Assignment" }, { value: "project", label: "Project" },
  { value: "internal-assessment", label: "Internal assessment" }, { value: "custom", label: "Custom" },
];
...
const formSchema = z
  .object({
    ...
    type: z.enum(["unit-test", "weekly-test", "monthly-test", "midterm", "half-yearly", "annual", "pre-board", "board", "practical", "oral", "assignment", "project", "internal-assessment", "custom"]),
  ...
  return (
    <div className="flex flex-col gap-md">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Define a new exam under a term</p>
      </div>

      <form onSubmit={form.handleSubmit(onSubmit)} className="flex max-w-xl flex-col gap-sm">
        {formError && <p className="rounded-md border border-error/30 bg-error/10 p-sm text-xs text-error">{formError}</p>}

        <div>
          <Label htmlFor="exam-name">Exam name</Label>
          <Input id="exam-name" placeholder="e.g. Half Yearly Examination" {...form.register("name")} />
        ...
        <div className="grid grid-cols-2 gap-sm">
          <div>
            ...
            <Input id="exam-code" placeholder="e.g. HY-2627" {...form.register("code")} />
          ...
          <div>
            ...
            <Controller control={form.control} name="type" render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                <SelectTrigger aria-label="Exam type"><SelectValue /></SelectTrigger>
                <SelectContent>{examTypeOptions.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
              </Select>
            )} />
          </div>
        </div>
        ...
        <div>
          ...
          <Controller control={form.control} name="examTermId" render={({ field }) => (
            <Select value={field.value} onValueChange={field.onChange}>
              <SelectTrigger aria-label="Term" id="exam-term"><SelectValue placeholder={(terms ?? []).length ? "Select term" : "No terms yet — create one first"} /></SelectTrigger>
        ...
        <div>
          ...
          <Textarea id="exam-desc" placeholder="Optional notes visible to staff" {...form.register("description")} />
        </div>
          ...
          <div>
            ...
            <Controller control={form.control} name="mode" render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                ...
                <SelectContent><SelectItem value="offline">Offline</SelectItem><SelectItem value="online">Online</SelectItem></SelectContent>
              </Select>
            )} />
          </div>
        </div>

        <p className="text-xs text-muted-foreground">Grading scheme, result rules and report card templates arrive with the Marks &amp; Results module.</p>

        <Button type="submit" disabled={busy} className="self-start">Create exam</Button>
      </form>
    </div>
  );
}
