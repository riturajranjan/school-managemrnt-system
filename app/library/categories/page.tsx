...
export default function CategoriesPage() {
  ...
  if (!can("library.view")) return <PermissionDenied action="view categories" role={roleLabels[role]} />;
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-md pb-20 sm:pb-0">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Classification and Dewey / custom codes</p>
      </div>

      {canManage && (
        <div className="flex flex-col gap-xs rounded-lg border border-border bg-surface p-md sm:flex-row sm:items-end">
          <div className="flex-1">
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Category name" aria-label="Category name" />
          </div>
          <div className="sm:w-32">
            <Input value={classification} onChange={(e) => setClassification(e.target.value)} placeholder="Dewey e.g. 500" aria-label="Classification" />
          </div>
          <Button onClick={add}>
            ...
            <div key={c.id} className="flex items-center justify-between gap-sm rounded-lg border border-border bg-surface p-sm">
              <div className="flex items-center gap-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{c.classification ? `Class ${c.classification}` : "No classification"}</p>
                </div>
              </div>
