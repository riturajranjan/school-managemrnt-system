...
import { addCopy, createBook, findDuplicateIsbn } from "@/lib/services/book-service";
...
export default function NewBookPage() {
  ...
  if (!can("library.manageCatalogue")) return <PermissionDenied action="add books to the catalogue" role={roleLabels[role]} />;
  ...
  function submit() {
    ...
    if (dupe && !confirmDupe) return setError(`A book with this ISBN already exists: "${dupe.title}". Confirm to add anyway.`);
    ...
    for (let i = 0; i < copyCount; i++) {
      addCopy({ bookId: result.book!.id, libraryId: result.book!.libraryId }, { name: "Librarian", role: roleLabels[role] });
    }
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-md pb-20 sm:pb-0">
      <div className="flex items-center gap-sm">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Create a catalogue record, then add physical copies</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-md rounded-lg border border-border bg-surface p-md">
        <div className="flex flex-col gap-1.5">
          ...
          <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Malgudi Days" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-md sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            ...
            {dupe && <p className="text-xs text-warning">Possible duplicate: “{dupe.title}”</p>}
          </div>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="accession">Accession number *</Label>
            <Input id="accession" value={accession} onChange={(e) => setAccession(e.target.value)} placeholder="ACC-00001" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-md sm:grid-cols-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input id="replacement" type="number" inputMode="numeric" value={replacement} onChange={(e) => setReplacement(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="copies">Initial copies</Label>
            <Input id="copies" type="number" inputMode="numeric" value={copies} onChange={(e) => setCopies(e.target.value)} />
          </div>
        </div>

        <label className="flex items-center justify-between gap-sm rounded-md border border-border p-sm">
          <span className="text-sm text-foreground">Reference only (not for lending)</span>
        ...
        {dupe && (
          <label className="flex items-center justify-between gap-sm rounded-md border border-warning/40 bg-warning/8 p-sm">
            <span className="text-sm text-warning">Add despite the possible ISBN duplicate</span>
