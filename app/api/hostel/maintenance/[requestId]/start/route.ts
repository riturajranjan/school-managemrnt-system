// POST /api/hostel/maintenance/:requestId/start — hostel.manage.
import type { NextRequest } from "next/server";
import { handle, requirePermission } from "@/lib/server/api/guard";
import { ok } from "@/lib/server/api/response";
import { requireOrgScope } from "@/lib/server/api/scope";
import { requireFeature } from "@/lib/server/platform/features-service";
import { startHostelMaintenance } from "@/lib/server/hostel/maintenance";
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  return handle(async () => {
    ...
    return ok(await startHostelMaintenance(scope, requestId));
  });
}
