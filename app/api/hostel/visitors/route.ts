// GET  /api/hostel/visitors — list, server-side search/filter/pagination. hostel.view.
// POST /api/hostel/visitors — create a visitor request for a real, in-scope resident. hostel.manage.
...
import { createHostelVisitor, listHostelVisitors } from "@/lib/server/hostel/visitors";
...
export async function GET(request: NextRequest) {
  return handle(async () => {
    ...
    const data = await listHostelVisitors(scope, {
...
export async function POST(request: NextRequest) {
  return handle(async () => {
    ...
    const scope = await requireOrgScope(ctx);
    await requireFeature(scope.schoolId, "hostel");
    return ok(await createHostelVisitor(scope, await readJson(request)));
  });
}
