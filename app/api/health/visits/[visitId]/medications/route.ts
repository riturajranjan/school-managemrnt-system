// POST /api/health/visits/[visitId]/medications — factual "medication given"
// record, never a prescription/dosage engine. Does not touch Inventory.
// health.manage.
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ visitId: string }> }) {
  return handle(async () => {
    ...
    return ok(await recordMedicationAdministration(scope, visitId, await readJson(request)));
  });
}
