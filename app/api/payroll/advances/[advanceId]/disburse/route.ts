// POST /api/payroll/advances/[advanceId]/disburse — APPROVED -> DISBURSED, recording only (never a real bank transfer). payroll.pay.
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ advanceId: string }> }) {
  return handle(async () => {
    ...
    const data = await disburseStaffFinancialAdvance(scope, "ADVANCE", advanceId, await readJson(request));
    return ok(data);
  });
}
