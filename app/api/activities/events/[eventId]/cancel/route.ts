// POST /api/activities/events/[eventId]/cancel — DRAFT|PUBLISHED -> CANCELLED. activities.manage.
import type { NextRequest } from "next/server";
import { handle, requirePermission } from "@/lib/server/api/guard";
import { ok } from "@/lib/server/api/response";
import { requireOrgScope } from "@/lib/server/api/scope";
import { requireFeature } from "@/lib/server/platform/features-service";
import { cancelEvent } from "@/lib/server/activities/events";
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  return handle(async () => {
    ...
    return ok(await cancelEvent(scope, eventId));
  });
}
