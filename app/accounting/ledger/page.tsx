...
export default function LedgerPage() {
  ...
  const entries = useMemo(
    () =>
      [...allLedgerEntries.filter((l) => l.ledgerRefId === accountId)].sort(
        (a, b) => (a.date < b.date ? -1 : 1),
      ),
    [allLedgerEntries, accountId],
  );
  ...
  const columns: ColumnDef<LedgerEntry>[] = [
    ...
    },
    {
      ...
      header: "Source",
      cell: (e) => (
        <Badge tone="info">{journalSourceTypeLabels[e.source]}</Badge>
      ),
    ...
    },
    {
      ...
      cell: (e) => (
        <span className="text-sm text-foreground">
          {e.debit.minorUnits > 0 ? formatMoney(e.debit) : "—"}
        </span>
      ),
    },
    {
      ...
      cell: (e) => (
        <span className="text-sm text-foreground">
          {e.credit.minorUnits > 0 ? formatMoney(e.credit) : "—"}
        </span>
      ),
    },
    {
    ...
    <div className="flex flex-col gap-md pb-20 sm:pb-0">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Running balance per account, replayed from every posted journal
        </p>
      </div>
      ...
      {account && (
        <div className="grid grid-cols-2 gap-sm sm:grid-cols-4">
          <StatTile
            label="Account type"
            value={chartOfAccountTypeLabels[account.type]}
            tone="neutral"
          />
          <StatTile
            label="Total debit"
            value={formatMoney(
              { minorUnits: totalDebit, currency: "INR" },
              { compact: true },
            )}
            tone="neutral"
          />
          <StatTile
            label="Total credit"
            value={formatMoney(
              { minorUnits: totalCredit, currency: "INR" },
              { compact: true },
            )}
            tone="neutral"
          />
          <StatTile
            ...
            value={formatMoney(balance, { compact: true })}
            tone={balance.minorUnits >= 0 ? "success" : "error"}
          />
        </div>
      )}
          ...
          <div className="surface-3d flex items-center justify-between gap-sm rounded-lg border border-border bg-surface p-sm">
            ...
            <div className="shrink-0 text-right">
              <p className="text-sm text-foreground">
                {e.debit.minorUnits > 0
                  ? `Dr ${formatMoney(e.debit)}`
                  : `Cr ${formatMoney(e.credit)}`}
              </p>
              <p className="text-xs text-muted-foreground">
                {formatMoney(e.runningBalance)}
              </p>
