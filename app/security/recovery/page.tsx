  ...
  return (
    <div className="mx-auto flex  flex-col gap-md pb-20 sm:pb-0">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="flex items-center gap-2 text-lg font-semibold text-foreground">
            <KeyRound className="size-5 text-primary" /> Recovery codes
          </h1>
          <p className="text-xs text-muted-foreground">
            Use these if you lose access to your second factor
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border bg-surface p-md">
        <div className="grid grid-cols-2 gap-2 font-mono text-sm">
          {codes.map((c) => (
            <span
              key={c}
              className="rounded-md border border-border bg-surface-secondary/40 px-3 py-1.5 text-center text-foreground">
              {c}
            </span>
          ))}
        </div>
        <div className="mt-md flex flex-wrap gap-xs">
          <Button
            ...
            {copied ? "Copied (simulation)" : "Copy all"}
          </Button>
          <Button
            size="sm"
            variant="outline"
      ...
      <p className="rounded-md border border-warning/30 bg-warning/8 p-sm text-xs text-warning">
        Demo mode — these are obviously mock codes and grant no real access.
