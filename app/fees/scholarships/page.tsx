...
// (kind: "scholarship"), the same canonical adjustment engine as Discounts
// and Late Fees — a distinct business concept, same real balance effect.
...
export default function ScholarshipsPage() {
  ...
  if (!canManage) {
    return (
      <div className="flex flex-col items-center gap-sm py-2xl text-center">
        ...
        <p className="text-sm text-muted-foreground">You don&apos;t have permission to manage scholarships.</p>
      </div>
    );
  }
  ...
  async function apply() {
    ...
    if (!reason.trim()) return setError("A reason is required (e.g. scholarship name).");
    setSaving(true);
    for (const chargeId of selected) {
      const res = await applyFeeAdjustmentRequest({ chargeId, kind: "scholarship", amountType, value: Number(value), reason: reason.trim() });
      if (!res.success) {
    ...
    <div className="flex flex-col gap-md pb-20 sm:pb-0">
      ...
      {report && (
        <div className="grid grid-cols-2 gap-sm sm:grid-cols-2">
          <StatTile label="Total scholarships awarded" value={formatCurrency(report.totalScholarships)} tone="neutral" />
          <StatTile label="Awards count" value={String(report.count)} tone="neutral" />
        </div>
      )}
        ...
        {studentId && ledger && (
          <div className="mt-sm flex flex-col gap-sm">
            ...
            <div className="grid grid-cols-1 gap-sm sm:grid-cols-3">
              ...
              <Input type="number" min={0} value={value} onChange={(e) => setValue(e.target.value)} placeholder={amountType === "percentage" ? "e.g. 25" : "e.g. 10000"} />
              <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Scholarship name (e.g. Merit Scholarship)" />
            </div>
