...
export default function NewJobPage() {
  ...
  function submit() {
    ...
    const r = createJob({
      ...
      responsibilities: [],
      requirements: [],
      deadline: new Date(Date.now() + 21 * 86_400_000)
        .toISOString()
        .slice(0, 10),
    });
      ...
      <div className="flex flex-col gap-md rounded-lg border border-border bg-surface p-md">
        ...
        <div className="flex justify-end gap-xs">
          <Button asChild variant="outline">
            <Link href="/hr/recruitment/jobs">Cancel</Link>
          </Button>
          <Button onClick={submit}>
            <BriefcaseBusiness className="size-4" /> Create job
          </Button>
        </div>
      </div>
    </div>
