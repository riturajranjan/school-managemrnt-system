...
import { studentFormSchema, type StudentFormValues } from "@/lib/schemas/student-form";
...
export default function NewStudentPage() {
  ...
  const form = useForm<StudentFormValues>({
    ...
    defaultValues: { gender: "prefer-not-to-say", admissionDate: new Date().toISOString().slice(0, 10) },
  });
  ...
  function onSubmit(values: StudentFormValues) {
    if (isDuplicateAdmissionNumber(db, values.admissionNumber)) {
      setDuplicateError(`Admission number "${values.admissionNumber}" is already in use.`);
      return;
    }
    ...
    const student: Student = {
      ...
      profile: { firstName: values.firstName, lastName: values.lastName, dob: values.dob, gender: values.gender, nationality: "Indian" },
      ...
      health: { emergencyContactName: values.guardianFirstName, emergencyContactPhone: values.guardianPhone, allergies: [], conditions: [], medications: [] },
      behaviourNotes: [],
      pulse: {
        overallScore: 75,
        status: "good",
      ...
      documents: [],
      timeline: [{ id: generateId("evt"), subjectId: studentId, category: "admission", title: "Student record created", actorName: "Administrator", createdAt: now }],
      createdAt: now,
      updatedAt: now,
    };
    ...
    setState((current) => ({
      ...
      guardians: [
        ...current.guardians,
        { id: guardianId, firstName: values.guardianFirstName, lastName: values.guardianLastName, contact: { phone: values.guardianPhone, email: values.guardianEmail || undefined }, communicationPreference: "sms" },
      ],
      studentGuardianLinks: [
        ...current.studentGuardianLinks,
        { studentId, guardianId, relationship: "guardian", isPrimary: true, isEmergencyContact: true, isAuthorizedPickup: true, isFeeResponsible: true },
      ],
    }));
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-md">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-md rounded-lg border border-border bg-surface p-md">
        {duplicateError && <p className="rounded-md border border-error/30 bg-error/10 p-sm text-xs text-error">{duplicateError}</p>}
        ...
        <div className="grid grid-cols-1 gap-sm sm:grid-cols-2">
          ...
          <div>
            ...
            <Input id="dob" type="date" {...form.register("dob")} aria-invalid={!!form.formState.errors.dob} />
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={(v) => { field.onChange(v); form.setValue("sectionId", ""); }}>
                  <SelectTrigger aria-label="Class" aria-invalid={!!form.formState.errors.classId}>
          ...
          <div>
            ...
            <Controller
              ...
              name="sectionId"
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange} disabled={sections.length === 0}>
                  <SelectTrigger aria-label="Section" aria-invalid={!!form.formState.errors.sectionId}>
        ...
        <div className="border-t border-border pt-md">
          ...
          <div className="grid grid-cols-1 gap-sm sm:grid-cols-2">
            ...
            <div>
              ...
              <Input id="guardianPhone" type="tel" {...form.register("guardianPhone")} aria-invalid={!!form.formState.errors.guardianPhone} />
