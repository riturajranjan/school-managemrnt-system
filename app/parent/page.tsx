...
export default function ParentAccountFoundationPage() {
  const { data, loading, error } = useApiResource<MyGuardianChild[]>("/api/me/guardian-children");

  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-md px-md py-3xl text-center">
      ...
      {loading ? (
        ...
        <>
          <h1 className="text-lg font-semibold text-foreground">Account not linked</h1>
          <p className="text-sm text-muted-foreground">{error}</p>
        </>
      ) : (
        <>
          <h1 className="text-lg font-semibold text-foreground">Welcome</h1>
          <p className="text-sm text-muted-foreground">Linked to {data?.length ?? 0} child{(data?.length ?? 0) === 1 ? "" : "ren"}:</p>
          <div className="flex w-full flex-col gap-xs">
            {data?.map((c) => (
              <div key={c.id} className="rounded-lg border border-border bg-surface p-sm text-left">
                <p className="text-sm font-medium text-foreground">{c.name}</p>
                <p className="text-xs text-muted-foreground">
                  {c.relation} · Admission No. {c.admissionNumber}
                  {c.classLabel ? ` · ${c.classLabel}${c.sectionLabel ? ` ${c.sectionLabel}` : ""}` : ""}
                </p>
              </div>
            ))}
          </div>
          <div className="mt-md rounded-lg border border-dashed border-border bg-surface px-md py-md text-sm text-muted-foreground">
            Your account is set up and this is a real login — but the parent portal
            (fees, attendance, transport, communication) is not built yet. It is a
            planned future phase, not something this page fakes.
          </div>
        </>
      )}
    </div>
