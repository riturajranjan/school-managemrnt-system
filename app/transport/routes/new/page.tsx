...
export default function NewRoutePage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { shift: "morning", direction: "both" },
  });

  if (!capabilitiesLoading && !hasServerPermission("transport.manage"))
    return (
      <PermissionDenied
  ...
  async function onSubmit(values: FormValues) {
    setSaving(true);
    setSaveError(null);
    const res = await createRouteRequest({
      name: values.name,
      code: values.code,
      shift: values.shift,
      ...
      <div className="flex items-center gap-sm">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Add stops and assign a vehicle/crew after creating
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-sm sm:grid-cols-2">
          <div>
            ...
            <Input
              id="rt-name"
              {...form.register("name")}
              placeholder="North Zone Morning"
            />
