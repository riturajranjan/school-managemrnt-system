...
export default function EmergencyPage() {
  ...
  if (!can("comm.emergency"))
    return (
      <PermissionDenied
        action="send emergency communications"
  ...
  return (
    <div className="mx-auto flex w-full  flex-col gap-md pb-20 sm:pb-0">
      <div className="flex items-center gap-sm rounded-lg border border-error/30 bg-error/5 p-md">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            High-priority alerts require confirmation before sending. Use
            responsibly.
          </p>
        </div>
      </div>
      ...
      {sent && (
        <div
          className="rounded-md border border-success/30 bg-success/8 p-sm text-sm text-success"
          role="status">
          Emergency notice published in-app. Demo channels (SMS/WhatsApp/Push)
          are not connected in this build. A follow-up message can be sent from
          Announcements.
        </div>
      )}
      ...
      <div className="flex flex-col gap-md rounded-lg border border-border bg-surface p-md">
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="emsg">Message</Label>
          <textarea
            id="emsg"
            value={message}
            ...
            placeholder="Clear, factual description of the situation…"
          />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="einstr">Instructions</Label>
          <Input
            id="einstr"
            value={instructions}
            onChange={(e) => setInstructions(e.target.value)}
            placeholder="e.g. Please collect your child from the main gate."
          />
        </div>
        <div className="flex flex-col gap-1.5">
        ...
        <div className="flex justify-end">
          <Button
            ...
            <Siren className="size-4" /> Review & send alert
          </Button>
        </div>
      </div>
