...
const ALL_SECTIONS: TemplateSectionType[] = [
  "logo",
  "school-name",
  "address",
  "contact",
...
const SAMPLE_ID = {
  id: "sample",
  cardNumber: "ID/STU/2026/001",
  kind: "student" as const,
  holderName: "Aarav Sharma",
  subtitle: "Class 5 · A",
  photoColor: "#18b0c8",
  style: "premium-teal" as const,
  issueDate: "2026-08-01",
  expiryDate: "2027-03-31",
  status: "issued" as const,
  verificationToken: "NVX-SAMPLE01",
  extra: {
    admissionNumber: "ADM-2026-001",
    guardian: "Mr. Sharma",
    guardianPhone: "+91 •••• ••1234",
    bloodGroup: "O+",
  },
};
...
export function TemplateBuilder({
  ...
  const update = (patch: Partial<DocumentTemplate>) => {
  ...
  const previewData: DocumentSheetData = {
    ...
    number: "SAMPLE/2026/001",
    accent: template.accent,
    recipientName: "Aarav Sharma",
    recipientSubtitle: "Class 5 · A",
    fields: {
      session: "2026-2027",
    ...
    signatoryName: "Dr. Meera Krishnan",
    token: "NVX-SAMPLE01",
    ...
    <div className="grid grid-cols-1 gap-md lg:grid-cols-[220px_minmax(0,1fr)_240px]">
      ...
      <div className="flex flex-col gap-sm">
        <div className="rounded-lg border border-border bg-surface p-sm">
          <p className="mb-sm flex items-center gap-1 text-xs font-semibold text-foreground">
            <LayoutList className="size-3.5" /> Sections in template
          </p>
          <div className="flex flex-col gap-1">
            {sortedSections.map((s) => (
              <button
                ...
                className={cn(
                  "flex items-center justify-between gap-1 rounded-md border px-2 py-1.5 text-left text-xs transition",
                  selectedId === s.id
          ...
          <div className="flex flex-wrap gap-1">
            {ALL_SECTIONS.filter((t) => !usedTypes.has(t)).map((t) => (
              <button
                ...
                className="rounded-pill bg-surface-secondary px-2 py-0.5 text-[11px] text-muted-foreground transition hover:text-foreground">
                + {sectionTypeLabels[t]}
              </button>
            ))}
          </div>
      ...
      <div className="flex flex-col gap-sm">
        <div className="flex items-center justify-between">
          <div className="flex gap-1 rounded-md border border-border bg-surface p-0.5">
            ...
            <button
              ...
              <LayoutList className="size-3.5" /> Structure
            </button>
          </div>
          <Button
            size="sm"
            ...
            disabled={issues.some((i) => i.level === "error")}>
            <Save className="size-3.5" /> {saved ? "Saved (mock)" : "Save"}
          </Button>
        </div>
            ...
            {issues.map((iss, i) => (
              <p
                ...
                className={cn(
                  ...
                  iss.level === "error" ? "text-error" : "text-warning",
                )}>
                <AlertTriangle className="size-3.5 shrink-0" /> {iss.message}
              </p>
            ))}
          </div>
        )}
        ...
        <div className="flex justify-center rounded-lg border border-border bg-surface-secondary/40 p-md">
          {view === "preview" ? (
            template.kind === "id-card" ? (
              <IdCard
                card={{ ...SAMPLE_ID, style: template.style ?? "premium-teal" }}
              />
            ) : (
              <DocumentSheet data={previewData} />
            )
          ) : (
              ...
              {sortedSections
                ...
                .map((s) => (
                  <div
                    key={s.id}
                    className={cn(
                      "rounded border border-dashed border-border px-2 py-1 text-xs",
                    ...
                    <span
                      className={cn(
                        s.fontWeight === "bold" && "font-bold",
                        s.fontWeight === "semibold" && "font-semibold",
                        s.fontWeight === "medium" && "font-medium",
                      )}>
                      {sectionTypeLabels[s.type]}
                    </span>
                  </div>
      ...
      <div className="flex flex-col gap-sm">
        <div className="rounded-lg border border-border bg-surface p-sm">
          <p className="mb-sm text-xs font-semibold text-foreground">
            Template
          </p>
          <div className="flex flex-col gap-sm">
            <div>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  update({ paperSize: v as DocumentTemplate["paperSize"] })
                }>
                <SelectTrigger aria-label="Paper size">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {Object.entries(paperSizeLabels).map(([k, v]) => (
            ...
            <p className="text-[11px] text-muted-foreground">
              {documentKindLabels[template.kind]} template
            </p>
          </div>
        </div>
        ...
        {selected ? (
          <div className="rounded-lg border border-border bg-surface p-sm">
            <p className="mb-sm text-xs font-semibold text-foreground">
              Section: {sectionTypeLabels[selected.type]}
            </p>
            <div className="flex flex-col gap-sm text-xs">
              <label className="flex items-center justify-between">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSection(selected.id, { show: e.target.checked })
                  }
                />
              </label>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSection(selected.id, {
                      align: v as TemplateSection["align"],
                    })
                  }>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSection(selected.id, {
                      fontSize: v as TemplateSection["fontSize"],
                    })
                  }>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSection(selected.id, {
                      fontWeight: v as TemplateSection["fontWeight"],
                    })
                  }>
                  ...
                  <SelectContent>
                    {["normal", "medium", "semibold", "bold"].map((f) => (
                      <SelectItem key={f} value={f}>
                        {f}
                      </SelectItem>
                    ))}
              ...
              <div>
                <Label htmlFor="sec-color">Text colour</Label>
                <input
                  id="sec-color"
                  type="color"
                  value={selected.color ?? "#111111"}
                  onChange={(e) =>
                    updateSection(selected.id, { color: e.target.value })
                  }
              ...
              {selected.type === "custom-text" && (
                <div>
                  <Label htmlFor="sec-custom">Custom text</Label>
                  <Input
                    id="sec-custom"
                    value={selected.customText ?? ""}
                    onChange={(e) =>
                      updateSection(selected.id, { customText: e.target.value })
                    }
                  />
                </div>
              )}
          ...
          <p className="rounded-lg border border-dashed border-border p-sm text-xs text-muted-foreground">
        ...
        <div className="rounded-lg border border-border bg-surface p-sm">
          <p className="mb-1 flex items-center gap-1 text-xs font-semibold text-foreground">
            <Info className="size-3.5" /> Variables
          </p>
          <div className="flex flex-wrap gap-1">
            {VARIABLES.map((v) => (
              <span
                key={v.key}
                title={`{{${v.key}}}`}
                className="rounded-pill bg-surface-secondary px-2 py-0.5 text-[10px] text-muted-foreground">
                {v.label}
              </span>
            ))}
          </div>
