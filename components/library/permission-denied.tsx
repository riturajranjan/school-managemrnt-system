...
export function PermissionDenied({ action, role, backHref = "/library" }: { action: string; role: string; backHref?: string }) {
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-sm rounded-lg border border-dashed border-border bg-surface px-md py-2xl text-center">
      ...
      <h1 className="text-base font-semibold text-foreground">Permission required</h1>
      <p className="text-sm text-muted-foreground">
        Your role ({role}) does not have permission to {action}. Contact an administrator if you need access.
      </p>
