    ...
    <div className="flex items-start gap-2 rounded-md border border-primary/25 bg-primary/5 p-sm text-xs text-foreground">
      ...
      <p>
        <span className="font-medium">
          Sensitive {kind === "counselling" ? "counselling" : "health"}{" "}
          information.
        </span>{" "}
        Access is restricted to authorised staff and recorded for audit. All
        records are explicit entries — nothing here is inferred, diagnosed or
        predicted.
      </p>
    </div>
  );
...
export function RestrictedHealth({
  ...
  return (
    <div className="mx-auto flex  flex-col items-center gap-sm rounded-lg border border-dashed border-border bg-surface px-md py-2xl text-center">
      ...
      <p className="text-sm text-muted-foreground">
        You can see that this record exists, but the detail is restricted to
        authorised staff. An access reason and audit entry would be required to
        view it.
      </p>
    </div>
  );
