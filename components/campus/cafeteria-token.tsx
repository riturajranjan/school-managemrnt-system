...
export function CafeteriaToken({ order }: { order: CafeteriaOrder }) {
  return (
    <div className="mx-auto w-full  overflow-hidden rounded-xl border border-border bg-surface shadow-floating">
      ...
      <div className="flex items-center gap-3 p-md">
        ...
        <div className="min-w-0 flex-1 text-xs text-muted-foreground">
          ...
          <p>
            Counter: <span className="text-foreground">{order.counter}</span>
          </p>
          <p>
            Pickup: <span className="text-foreground">{order.pickupTime}</span>
          </p>
          <p>
            Total:{" "}
          ...
          <span
            className={cn(
              "mt-1 inline-flex rounded-pill px-2 py-0.5 text-[11px] font-medium",
              toneClasses[orderStatusTone[order.status]].soft,
            )}>
      ...
      <p className="px-md pb-2 text-[10px] text-muted-foreground">
        QR encodes an opaque token only — no personal data.
      </p>
    </div>
  );
}
