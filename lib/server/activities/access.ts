// Shared helpers for Activities / Student Life (Phase 9U). A member is
// always a real, active Student.id; a coordinator/coach/mentor is always a
// real, active Staff.id — never a parallel identity.
...
export async function resolveActivityBranch(scope: OrgScope): Promise<string> {
  ...
  throw new HttpError("INVALID_BRANCH", "Select a branch for activities");
}
...
export async function requireValidStudent(scope: OrgScope, studentId: string): Promise<{ id: string; branchId: string }> {
  ...
  if (!student) throw new HttpError("INVALID_ACTIVITY_STUDENT", "Student must be a real, active student in this school");
  return student;
}

export async function requireValidStaff(scope: OrgScope, staffId: string): Promise<{ id: string }> {
  const staff = await prisma.staff.findFirst({ where: { id: staffId, schoolId: scope.schoolId, status: "ACTIVE" }, select: { id: true } });
  if (!staff) throw new HttpError("INVALID_ACTIVITY_STAFF", "Coordinator must be a real, active staff member in this school");
  return staff;
}
