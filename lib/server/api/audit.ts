// Lightweight audit recorder (spec §30). Records important business actions into
// the AuditEvent table with ids + a small meta blob only — never bulky PII.
// Accepts a Prisma client OR a transaction client so it can participate in the
// same transaction as the action it records.
import type { Prisma } from "@/lib/generated/prisma/client";
import type { OrgScope } from "./scope";

export type AuditAction =
  | "STUDENT_CREATED"
  | "STUDENT_UPDATED"
  | "STUDENT_ARCHIVED"
  | "STUDENT_RESTORED"
  | "GUARDIAN_CREATED"
  | "GUARDIAN_UPDATED"
  | "GUARDIAN_LINKED"
  | "GUARDIAN_UNLINKED"
  | "ADMISSION_CREATED"
  | "ADMISSION_UPDATED"
  | "ADMISSION_STAGE_CHANGED"
  | "ADMISSION_APPROVED"
  | "ADMISSION_REJECTED"
  | "ADMISSION_CONVERTED"
  // Platform (Super Admin) actions — actor is a platform admin; tenantId/schoolId
  // point at the TARGET tenant/school, never the actor.
  | "SCHOOL_CREATED"
  | "SCHOOL_UPDATED"
  | "SCHOOL_SUSPENDED"
  | "SCHOOL_REACTIVATED"
  | "ONBOARDING_UPDATED"
  | "ONBOARDING_COMPLETED"
  | "TRIAL_EXTENDED"
  | "TRIAL_CONVERTED"
  | "TRIAL_ENDED"
  | "INVOICE_GENERATED"
  | "INVOICE_ISSUED"
  | "INVOICE_PAID"
  | "INVOICE_VOIDED"
  | "PAYMENT_RECORDED"
  | "PAYMENT_REVERSED"
  | "SUPPORT_TICKET_CREATED"
  | "SUPPORT_TICKET_ASSIGNED"
  | "SUPPORT_TICKET_STATUS_CHANGED"
  | "SUPPORT_MESSAGE_ADDED"
  | "SUPPORT_NOTE_ADDED"
  | "IMPERSONATION_STARTED"
  | "IMPERSONATION_ENDED"
  | "FEATURE_OVERRIDE_SET"
  | "FEATURE_OVERRIDE_CLEARED"
  | "DOMAIN_ADDED"
  | "DOMAIN_STATUS_CHANGED"
  | "DOMAIN_REMOVED"
  | "BRANDING_UPDATED"
  | "ADDON_CREATED"
  | "ADDON_UPDATED"
  | "ADDON_STATUS_CHANGED"
  | "ADDON_ASSIGNED"
  | "ADDON_REMOVED"
  | "MARKETPLACE_APP_CREATED"
  | "MARKETPLACE_APP_UPDATED"
  | "MARKETPLACE_APP_STATUS_CHANGED"
  | "MARKETPLACE_APP_INSTALLED"
  | "MARKETPLACE_APP_DISABLED"
  | "PLATFORM_SETTINGS_UPDATED"
  | "PLATFORM_ADMIN_INVITED"
  | "PLATFORM_ADMIN_UPDATED"
  | "PLATFORM_ADMIN_STATUS_CHANGED"
  | "ANNOUNCEMENT_CREATED"
  | "ANNOUNCEMENT_UPDATED"
  | "ANNOUNCEMENT_PUBLISHED"
  | "ANNOUNCEMENT_ARCHIVED"
  | "PLATFORM_INCIDENT_CREATED"
  | "PLATFORM_INCIDENT_UPDATED"
  | "PLATFORM_INCIDENT_RESOLVED"
  | "CLASS_CREATED"
  | "CLASS_UPDATED"
  | "SECTION_CREATED"
  | "SECTION_UPDATED"
  | "STUDENT_ENROLLED"
  | "STUDENT_UNENROLLED"
  | "SUBJECT_CREATED"
  | "SUBJECT_UPDATED"
  | "SUBJECT_ARCHIVED"
  | "SUBJECT_RESTORED"
  | "SUBJECT_DUPLICATED"
  | "CLASS_SUBJECT_ASSIGNED"
  | "CLASS_SUBJECT_REMOVED"
  | "CLASS_SUBJECTS_UPDATED"
  | "STAFF_CREATED"
  | "STAFF_UPDATED"
  | "STAFF_STATUS_CHANGED"
  | "STAFF_USER_LINKED"
  | "STAFF_USER_UNLINKED"
  | "HR_DEPARTMENT_CREATED"
  | "HR_DEPARTMENT_UPDATED"
  | "HR_DEPARTMENT_ARCHIVED"
  | "HR_DESIGNATION_CREATED"
  | "HR_DESIGNATION_UPDATED"
  | "HR_DESIGNATION_ARCHIVED"
  | "HOSTEL_CREATED"
  | "HOSTEL_UPDATED"
  | "HOSTEL_ROOM_CREATED"
  | "HOSTEL_ROOM_UPDATED"
  | "HOSTEL_BED_UPDATED"
  | "HOSTEL_STUDENT_ASSIGNED"
  | "HOSTEL_STUDENT_TRANSFERRED"
  | "HOSTEL_STUDENT_VACATED"
  | "HOSTEL_STAFF_ASSIGNED"
  | "HOSTEL_STAFF_ASSIGNMENT_ENDED"
  | "HOSTEL_ROLL_CALL_MARKED"
  | "HOSTEL_LEAVE_REQUESTED"
  | "HOSTEL_LEAVE_APPROVED"
  | "HOSTEL_LEAVE_REJECTED"
  | "HOSTEL_LEAVE_CANCELLED"
  | "HOSTEL_VISITOR_REQUESTED"
  | "HOSTEL_VISITOR_CHECKED_IN"
  | "HOSTEL_VISITOR_CHECKED_OUT"
  | "HOSTEL_VISITOR_CANCELLED"
  | "HOSTEL_COMPLAINT_CREATED"
  | "HOSTEL_COMPLAINT_ASSIGNED"
  | "HOSTEL_COMPLAINT_STATUS_CHANGED"
  | "HOSTEL_COMPLAINT_RESOLVED"
  | "HOSTEL_MAINTENANCE_CREATED"
  | "HOSTEL_MAINTENANCE_ASSIGNED"
  | "HOSTEL_MAINTENANCE_STATUS_CHANGED"
  | "HOSTEL_MAINTENANCE_COMPLETED"
  | "TEACHING_ASSIGNMENT_CREATED"
  | "TEACHING_ASSIGNMENT_REMOVED"
  | "TIMETABLE_PERIODS_UPDATED"
  | "TIMETABLE_ENTRY_CREATED"
  | "TIMETABLE_ENTRY_UPDATED"
  | "TIMETABLE_ENTRY_DELETED"
  | "EXAM_TERM_CREATED"
  | "EXAM_TERM_UPDATED"
  | "EXAM_TERM_STATUS_CHANGED"
  | "EXAM_CREATED"
  | "EXAM_UPDATED"
  | "EXAM_STATUS_CHANGED"
  | "EXAM_CLASS_ASSIGNED"
  | "EXAM_CLASS_REMOVED"
  | "EXAM_SCHEDULE_CREATED"
  | "EXAM_SCHEDULE_UPDATED"
  | "EXAM_SCHEDULE_DELETED"
  | "EXAM_MARKS_SAVED"
  | "EXAM_MARKS_SUBMITTED"
  | "EXAM_MARKS_VERIFIED"
  | "GRADING_SCHEME_CREATED"
  | "GRADING_SCHEME_UPDATED"
  | "GRADING_BANDS_UPDATED"
  | "EXAM_RESULTS_PUBLISHED"
  | "ATTENDANCE_SESSION_CREATED"
  | "ATTENDANCE_UPDATED"
  | "ATTENDANCE_SUBMITTED"
  | "ATTENDANCE_LOCKED"
  | "STUDENT_PROMOTED"
  | "STUDENT_RETAINED"
  | "HOMEWORK_CREATED"
  | "HOMEWORK_UPDATED"
  | "HOMEWORK_PUBLISHED"
  | "HOMEWORK_CLOSED"
  | "CURRICULUM_CREATED"
  | "CURRICULUM_UPDATED"
  | "CURRICULUM_STATUS_CHANGED"
  | "CURRICULUM_UNIT_CREATED"
  | "CURRICULUM_UNIT_UPDATED"
  | "CURRICULUM_CHAPTER_CREATED"
  | "CURRICULUM_CHAPTER_UPDATED"
  | "CURRICULUM_TOPIC_CREATED"
  | "CURRICULUM_TOPIC_UPDATED"
  | "CURRICULUM_TOPIC_PROGRESS_UPDATED"
  | "LESSON_PLAN_CREATED"
  | "LESSON_PLAN_UPDATED"
  | "LESSON_PLAN_STATUS_CHANGED"
  | "LESSON_PLAN_COMPLETED"
  | "CALENDAR_EVENT_CREATED"
  | "CALENDAR_EVENT_UPDATED"
  | "CALENDAR_EVENT_CANCELLED"
  | "STAFF_ATTENDANCE_MARKED"
  | "STAFF_ATTENDANCE_UPDATED"
  | "LEAVE_TYPE_CREATED"
  | "LEAVE_TYPE_UPDATED"
  | "LEAVE_REQUEST_CREATED"
  | "LEAVE_REQUEST_APPROVED"
  | "LEAVE_REQUEST_REJECTED"
  | "LEAVE_REQUEST_CANCELLED"
  | "FEE_CATEGORY_CREATED"
  | "FEE_CATEGORY_UPDATED"
  | "FEE_STRUCTURE_CREATED"
  | "FEE_STRUCTURE_UPDATED"
  | "FEE_STRUCTURE_STATUS_CHANGED"
  | "FEE_ASSIGNED"
  | "FEE_PAYMENT_RECORDED"
  | "FEE_DISCOUNT_APPLIED"
  | "FEE_SCHOLARSHIP_APPLIED"
  | "FEE_LATE_FEE_APPLIED"
  | "FEE_REFUND_CREATED"
  | "FEE_PAYMENT_RECONCILED"
  | "ACCOUNT_CREATED"
  | "ACCOUNT_UPDATED"
  | "ACCOUNT_STATUS_CHANGED"
  | "JOURNAL_CREATED"
  | "JOURNAL_POSTED"
  | "JOURNAL_REVERSED"
  | "SALARY_COMPONENT_CREATED"
  | "SALARY_COMPONENT_UPDATED"
  | "SALARY_STRUCTURE_CREATED"
  | "SALARY_STRUCTURE_UPDATED"
  | "STAFF_SALARY_ASSIGNED"
  | "PAYROLL_RUN_CREATED"
  | "PAYROLL_CALCULATED"
  | "PAYROLL_FINALIZED"
  | "PAYROLL_PAYMENT_RECORDED"
  | "VISITOR_CREATED"
  | "VISIT_EXPECTED"
  | "VISITOR_CHECKED_IN"
  | "VISITOR_CHECKED_OUT"
  | "VISIT_CANCELLED"
  | "CONVERSATION_CREATED"
  | "MESSAGE_SENT"
  | "TRANSPORT_VEHICLE_CREATED"
  | "TRANSPORT_VEHICLE_UPDATED"
  | "TRANSPORT_ROUTE_CREATED"
  | "TRANSPORT_ROUTE_UPDATED"
  | "TRANSPORT_STOP_CREATED"
  | "TRANSPORT_STOP_UPDATED"
  | "TRANSPORT_ROUTE_ASSIGNMENT_CREATED"
  | "TRANSPORT_ROUTE_ASSIGNMENT_ENDED"
  | "TRANSPORT_STUDENT_ASSIGNED"
  | "TRANSPORT_STUDENT_UNASSIGNED"
  | "TRANSPORT_STAFF_ASSIGNED"
  | "TRANSPORT_STAFF_UNASSIGNED"
  | "TRANSPORT_TRIP_CREATED"
  | "TRANSPORT_TRIP_STARTED"
  | "TRANSPORT_TRIP_COMPLETED"
  | "TRANSPORT_TRIP_CANCELLED"
  | "TRANSPORT_STUDENT_BOARDED"
  | "TRANSPORT_STUDENT_DROPPED"
  | "TRANSPORT_INCIDENT_REPORTED"
  | "TRANSPORT_INCIDENT_STATUS_CHANGED"
  | "TRANSPORT_MAINTENANCE_SCHEDULED"
  | "TRANSPORT_MAINTENANCE_STARTED"
  | "TRANSPORT_MAINTENANCE_COMPLETED"
  | "TRANSPORT_MAINTENANCE_CANCELLED"
  | "TRANSPORT_FUEL_LOGGED"
  | "TRANSPORT_DOCUMENT_ADDED"
  | "TRANSPORT_NOTIFICATION_SENT"
  | "LIBRARY_BOOK_CREATED"
  | "LIBRARY_BOOK_UPDATED"
  | "LIBRARY_COPY_CREATED"
  | "LIBRARY_COPY_UPDATED"
  | "LIBRARY_BOOK_ISSUED"
  | "LIBRARY_BOOK_RENEWED"
  | "LIBRARY_BOOK_RETURNED"
  | "LIBRARY_COPY_MARKED_LOST"
  | "LIBRARY_COPY_MARKED_DAMAGED"
  | "LIBRARY_POLICY_UPDATED"
  | "LIBRARY_DIGITAL_RESOURCE_CREATED"
  | "LIBRARY_DIGITAL_RESOURCE_DELETED"
  | "LIBRARY_STOCKTAKE_STARTED"
  | "LIBRARY_STOCKTAKE_COMPLETED"
  | "INVENTORY_ITEM_CREATED"
  | "INVENTORY_ITEM_UPDATED"
  | "INVENTORY_LOCATION_CREATED"
  | "INVENTORY_STOCK_RECEIVED"
  | "INVENTORY_STOCK_ISSUED"
  | "INVENTORY_STOCK_RETURNED"
  | "INVENTORY_STOCK_TRANSFERRED"
  | "INVENTORY_STOCK_ADJUSTED"
  | "ASSET_CREATED"
  | "ASSET_UPDATED"
  | "ASSET_ASSIGNED"
  | "ASSET_RETURNED"
  | "ASSET_STATUS_CHANGED"
  | "ASSET_MAINTENANCE_OPENED"
  | "ASSET_MAINTENANCE_COMPLETED"
  | "ASSET_DISPOSED"
  | "HEALTH_PROFILE_UPDATED"
  | "HEALTH_VISIT_CREATED"
  | "HEALTH_VISIT_UPDATED"
  | "HEALTH_VISIT_CLOSED"
  | "HEALTH_VISIT_REFERRED"
  | "HEALTH_VITAL_RECORDED"
  | "HEALTH_TREATMENT_RECORDED"
  | "HEALTH_MEDICATION_RECORDED"
  | "COUNSELING_CASE_CREATED"
  | "COUNSELING_CASE_ASSIGNED"
  | "COUNSELING_CASE_UPDATED"
  | "COUNSELING_CASE_CLOSED"
  | "COUNSELING_SESSION_CREATED"
  | "COUNSELING_NOTE_CREATED"
  | "COUNSELING_FOLLOWUP_UPDATED"
  | "CAFETERIA_LOCATION_CREATED"
  | "CAFETERIA_LOCATION_UPDATED"
  | "CAFETERIA_ITEM_CREATED"
  | "CAFETERIA_ITEM_UPDATED"
  | "CAFETERIA_MENU_CREATED"
  | "CAFETERIA_MENU_UPDATED"
  | "CAFETERIA_MEAL_SERVED"
  | "ACTIVITY_CREATED"
  | "ACTIVITY_UPDATED"
  | "ACTIVITY_STATUS_CHANGED"
  | "ACTIVITY_STAFF_ASSIGNED"
  | "ACTIVITY_STAFF_ENDED"
  | "ACTIVITY_STUDENT_JOINED"
  | "ACTIVITY_STUDENT_LEFT"
  | "ACTIVITY_EVENT_CREATED"
  | "ACTIVITY_EVENT_UPDATED"
  | "ACTIVITY_EVENT_PUBLISHED"
  | "ACTIVITY_EVENT_COMPLETED"
  | "ACTIVITY_EVENT_CANCELLED"
  | "ACTIVITY_PARTICIPANT_REGISTERED"
  | "ACTIVITY_PARTICIPANT_UPDATED"
  | "ACTIVITY_ACHIEVEMENT_RECORDED"
  | "DOCUMENT_TEMPLATE_CREATED"
  | "DOCUMENT_TEMPLATE_UPDATED"
  | "DOCUMENT_TEMPLATE_ACTIVATED"
  | "DOCUMENT_TEMPLATE_ARCHIVED"
  | "DOCUMENT_GENERATED"
  | "DOCUMENT_VOIDED"
  // Phase 9W.2 — hierarchical account provisioning. Metadata carries only ids/
  // role keys/booleans — never a password, raw setup token, or session token.
  | "USER_ACCOUNT_PROVISIONED"
  | "USER_ROLE_ASSIGNED"
  | "USER_ACCOUNT_LINKED"
  | "USER_ACCOUNT_ACTIVATED"
  | "USER_ACCOUNT_SUSPENDED"
  // User Account Creation Foundation review — admin-triggered password reset
  // (reissues a real one-time setup link; never carries a password/token).
  | "PASSWORD_RESET"
  | "PASSWORD_CHANGED"
  // Student self-service — Add/Invite My Guardian (identity-scoped, never a
  // client-supplied studentId).
  | "GUARDIAN_INVITED_BY_STUDENT"
  // Production Accounting checkpoint — Vendors / Purchase Orders / Budgets.
  | "VENDOR_CREATED"
  | "VENDOR_UPDATED"
  | "VENDOR_STATUS_CHANGED"
  | "PURCHASE_ORDER_CREATED"
  | "PURCHASE_ORDER_APPROVED"
  | "PURCHASE_ORDER_CANCELLED"
  | "BUDGET_CREATED"
  | "BUDGET_APPROVED"
  // Production Payroll checkpoint — Loans / Advances.
  | "STAFF_FINANCIAL_ADVANCE_CREATED"
  | "STAFF_FINANCIAL_ADVANCE_UPDATED"
  | "STAFF_FINANCIAL_ADVANCE_APPROVED"
  | "STAFF_FINANCIAL_ADVANCE_REJECTED"
  | "STAFF_FINANCIAL_ADVANCE_CANCELLED"
  | "STAFF_FINANCIAL_ADVANCE_DISBURSED"
  | "STAFF_FINANCIAL_ADVANCE_REPAYMENT_RECORDED"
  // Production migration (Phase B, HR Sub-batch 2) — Contracts / Documents.
  | "CONTRACT_CREATED"
  | "CONTRACT_UPDATED"
  | "CONTRACT_STATUS_CHANGED"
  | "STAFF_DOCUMENT_UPLOADED"
  | "STAFF_DOCUMENT_VERIFIED"
  | "STAFF_DOCUMENT_REJECTED"
  | "STAFF_DOCUMENT_ARCHIVED"
  // Production migration (Phase B, HR Sub-batch 3) — Performance / Training.
  | "PERFORMANCE_REVIEW_CREATED"
  | "PERFORMANCE_REVIEW_UPDATED"
  | "PERFORMANCE_REVIEW_STATUS_CHANGED"
  | "TRAINING_PROGRAM_CREATED"
  | "TRAINING_PROGRAM_UPDATED"
  | "TRAINING_PROGRAM_STATUS_CHANGED"
  | "TRAINING_PARTICIPANT_ASSIGNED"
  | "TRAINING_PARTICIPANT_STATUS_CHANGED"
  // Production migration (Phase B, HR Sub-batch 4) — Recruitment / Onboarding / Policies / Shifts.
  | "JOB_OPENING_CREATED"
  | "JOB_OPENING_UPDATED"
  | "JOB_OPENING_STATUS_CHANGED"
  | "JOB_APPLICANT_CREATED"
  | "JOB_APPLICANT_UPDATED"
  | "JOB_APPLICANT_STAGE_CHANGED"
  | "EMPLOYEE_ONBOARDING_STARTED"
  | "EMPLOYEE_ONBOARDING_UPDATED"
  | "EMPLOYEE_ONBOARDING_STATUS_CHANGED"
  | "ONBOARDING_TASK_COMPLETED"
  | "ONBOARDING_TASK_REOPENED"
  | "HR_POLICY_CREATED"
  | "HR_POLICY_UPDATED"
  | "HR_POLICY_STATUS_CHANGED"
  | "HR_POLICY_ACKNOWLEDGED"
  | "SHIFT_CREATED"
  | "SHIFT_UPDATED"
  | "SHIFT_STATUS_CHANGED"
  | "SHIFT_ASSIGNED";

export async function recordAudit(
  db: Prisma.TransactionClient,
  scope: OrgScope,
  action: AuditAction,
  entityType: string,
  entityId: string,
  meta?: Record<string, unknown>,
): Promise<void> {
  await db.auditEvent.create({
    data: {
      tenantId: scope.tenantId,
      schoolId: scope.schoolId,
      actorUserId: scope.actor.id,
      actorName: scope.actor.name,
      actorRoleKey: scope.actor.roleKey ?? null,
      action,
      entityType,
      entityId,
      metaJson: (meta as Prisma.InputJsonValue | undefined) ?? undefined,
    },
  });
}
